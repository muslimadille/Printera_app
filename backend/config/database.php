...
return [
    ...
    */
    ...
    'connections' => [
        ...
        /*
        |----------------------------------------------------------------------
        | Supabase (migration source — Phase 7 only)
        |----------------------------------------------------------------------
        | Read-only source for `app:migrate-from-supabase`. Nothing in the running
        | application touches this connection; it exists so the migration command can
        | stream rows straight across instead of round-tripping a dump file.
        |
        | Supabase's pooler requires TLS, hence sslmode=require by default. Take the
        | values from Settings → Database → Connection string (use the DIRECT connection,
        | not the transaction pooler — the copy runs long transactions).
        |
        | Delete this block, and the SUPABASE_DB_* vars, in Phase 8.
        */
        'supabase' => [
            'driver' => 'pgsql',
