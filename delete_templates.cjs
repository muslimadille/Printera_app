...
if (fs.existsSync(tabRegistryPath)) {
...
if (fs.existsSync(appTabsPath)) {
  ...
  renderItemsToRemove.forEach(r => {
    // using string replacement for safety, but with indentation it might fail.
    // better use regex.
    const key = r.match(/'(.*?)'/)[1];
    appTabsContent = appTabsContent.replace(new RegExp(`\\s*\\{\\s*key:\\s*'${key}'.*?\\},?\\n`, 'g'), '\n');
  });

  const showsToRemove = [
    "showBD003", "showBT0001", "showBT0003", "showBT0004", "showBT0007", "showBT0009", "showBA01010000", "showBA01700000", "showBGeneric"
  ];
  showsToRemove.forEach(r => {
    appTabsContent = appTabsContent.replace(new RegExp(`\\s*const ${r} = shouldRenderTab\\('.*?'\\);\\n`, 'g'), '\n');
  });

  const blocksToRemove = [
    /<div hidden=\{effectiveBSub !== 'box_d003'\}[\s\S]*?<\/div>\s*/g,
    /<div hidden=\{effectiveBSub !== 'box_t0001'\}[\s\S]*?<\/div>\s*/g,
    /<div hidden=\{effectiveBSub !== 'box_t0003'\}[\s\S]*?<\/div>\s*/g,
    /<div hidden=\{effectiveBSub !== 'box_t0004'\}[\s\S]*?<\/div>\s*/g,
    /<div hidden=\{effectiveBSub !== 'box_t0007'\}[\s\S]*?<\/div>\s*/g,
    /<div hidden=\{effectiveBSub !== 'box_t0009'\}[\s\S]*?<\/div>\s*/g,
    /<div hidden=\{effectiveBSub !== 'box_a01010000'\}[\s\S]*?<\/div>\s*/g,
    /<div hidden=\{effectiveBSub !== 'box_a01700000'\}[\s\S]*?<\/div>\s*/g,
    /<div hidden=\{effectiveBSub !== 'box_generic'\}[\s\S]*?<\/div>\s*/g,
  ];
