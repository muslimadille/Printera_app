...
## 7d. Phase 7 prep — cross-engine findings (OPS-070a / OPS-071a)

Phase 7's tooling was written and rehearsed **PostgreSQL → PostgreSQL**, before BE-060 made
MySQL the production target. Re-pointed at the real pairing it copied **zero rows**. These
are the deviations that came out of fixing that.

### Timestamps are re-rendered, and that is the one exception to "copy verbatim"
· **found by OPS-070a**

`SQLSTATE[22007]: 1292 Incorrect datetime value: '2026-04-05 09:00:00+00' for column
'created_at'` — all seven tables. Postgres prints `timestamptz` with an offset and
microseconds; MySQL's strict-mode parser accepts neither.

The copy now reads the offset, converts to UTC, and writes a spelling the target accepts
(offset-bearing for a Postgres target, naive for MySQL/SQLite). The **instant is
preserved**; only its spelling changes. Reading the offset rather than stripping it also
makes the copy independent of the source session's `TimeZone`, which is not something the
runbook can control on a hosted Supabase instance.

Sub-second precision is **floored, not rounded**. Every timestamp column in this schema is
precision 0 on all three engines — including Postgres, where `timestampTz()` also defaults
to 0 — so the fraction cannot be stored anywhere. Rounding is MySQL's own default and can
move an event into the following second, or day.

### The test fixture was proving nothing · **process finding**

`MigrateFromSupabaseTest` built its "Supabase" source by running **this app's own
migrations** on a second connection. Source and target were therefore identical by
construction, and every cross-engine difference the command exists to bridge was invisible.
The fixture is now `backend/tests/Fixtures/supabase-source.sql`, transcribed from
`supabase/migrations/*.sql` — `text`, `jsonb`, `timestamptz`, `uuid`, and Supabase's own
column order — and the MySQL and PostgreSQL lanes point it at a real PostgreSQL instance.

Generalises past this ticket: **a fixture generated by the code under test cannot falsify
it.** The same shape is worth checking wherever a test builds its own input from
production code.

### The pre-flight refuses rather than half-copying · **addition**

Supabase types every string column as unbounded `text`; BE-060 had to bound the ones MySQL
indexes. `app:migrate-from-supabase` now scans the source before writing anything and
aborts on (a) a source column the target lacks — silent data loss, unrecoverable once
Supabase is retired — or (b) a value too long for its target column.

**`login_logs.ip_address` is expected to fire in production.** It is `varchar(45)`, and
`manage-users/index.ts:146` stores `x-forwarded-for` raw, which behind a proxy is a
comma-separated chain. `--truncate-overlong` trims and prints an audit list of every row
touched; the leftmost entry of such a chain is the client IP, so the part the admin panel
shows is the part kept. Widening the column is the alternative. Deliberately not a default
either way — see PHASE-7-RUNBOOK §3.

### `config/database.php` now pins the MySQL session time zone · **hardening**

MySQL converts `TIMESTAMP` between the session zone and UTC on every read and write, and
all but `expires_at` are `TIMESTAMP`. With no `timezone` key Laravel emits no
`SET time_zone`, so the meaning of every stored audit timestamp depended on the host's
default. Now `+00:00`. This affects the running application, not only the migration — the
app already writes UTC and was relying on an unstated deployment invariant.

### The storage source is an interface · **addition (OPS-071a)**

`app:migrate-supabase-storage` read the Supabase REST API directly, so its only test was
against hand-written fake responses — the test and the code encoding one author's model of
that API. The source is now `App\Services\Storage\ObjectSource`, with the API walk and a
plain-disk reader behind it. Tests copy real bytes from a real (faked) disk; the API tests
are unchanged and still pass, which is the evidence the extraction preserved behaviour.

It also bought an operational route: `--from-disk` migrates from a bucket **export**, so
the service-role key never has to leave the dashboard, and the export doubles as the
storage backup the runbook requires.
