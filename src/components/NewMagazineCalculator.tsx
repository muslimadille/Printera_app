...
import { Plus, Trash2, Calculator, BookOpen, ChevronDown, Sparkles, Copy, Save, AlertTriangle } from 'lucide-react';
...
function roundUpTo4(n: number): number {
...
interface PrintRow {
  ...
  pressHeight: number;
  /** Stage 1 scenario id chosen by user */
  selectedStage1Id?: string;
  /** Stage 2 scenario id chosen by user */
...
function calcRow(
  ...
  // For magazine: cutsPerSheet is based on number of sheets (not pages)
  ...
  let sortCost = 0;
  if (cp) sortCost = (row.printedFaces === 2 && row.facesDifferent) ? cp.sortPerFace * 2 : cp.sortPerFace;

  let printCost = 0;
  if (cp) printCost = row.printedFaces * (cp.printFirst1000PerFace + Math.max(0, thousands - 1) * cp.printExtra1000PerFace);
  ...
  let dieCutCost = 0;
  if (row.hasDieCut && !isHidden('diecut')) dieCutCost = size.diecut1st1000 + Math.max(0, thousands - 1) * size.diecutExtra1000;

  let paperCost = 0;
  if (row.paperType && row.grammage && row.purchaseSize) {
    const selectedType = paperTypes.find(t => t.name === row.paperType);
    const selectedEntry = selectedType?.entries.find(e => e.sizeName === row.purchaseSize && e.grammage === row.grammage);
    if (selectedEntry) {
  ...
  let finishingCost = 0;
  row.finishing.forEach(f => { finishingCost += calculateFinishingCost(f, copies, thousands); });
    ...
    <div className="flex items-center gap-2 flex-wrap">
      ...
      <div className="flex items-center gap-1.5 flex-wrap w-full sm:w-auto flex-1">
        ...
        <div className="flex items-center gap-1.5 w-full sm:w-auto">
          <Input type="number" className="h-8 text-xs flex-1 min-w-0 bg-background" value={item.multiplier} onChange={(e) => onChange({ multiplier: Number(e.target.value) })} onClick={(e) => e.stopPropagation()} />
          <Input type="number" className="h-8 text-xs flex-1 min-w-0 bg-background" value={item.pricePerUnit} onChange={(e) => onChange({ pricePerUnit: Number(e.target.value) })} onClick={(e) => e.stopPropagation()} />
          {item.calcType === 'tiered_1000' && (
            <Input type="number" className="h-8 text-xs flex-1 min-w-0 bg-background" value={item.extraPer1000} onChange={(e) => onChange({ extraPer1000: Number(e.target.value) })} onClick={(e) => e.stopPropagation()} />
          )}
        </div>
...
const NewMagazineCalculator = ({ onNavigateToQuote, sessionToken }: { onNavigateToQuote?: () => void; sessionToken?: string }) => {
  ...
  const [totalPagesInput, setTotalPagesInput] = useState(16);
  ...
  useState(() => {
    if (editingQuoteData?.sourceType === 'newmagazine' && editingQuoteData.rawInputs) {
      ...
      if (raw.totalPagesInput) setTotalPagesInput(raw.totalPagesInput);
      if (raw.coverIncluded !== undefined) setCoverIncluded(raw.coverIncluded);
  ...
  // Calculate actual pages (rounded to 4) and sheets
  const actualPages = roundUpTo4(totalPagesInput);
  const sheetsPerCopy = actualPages / 4;
  const sheetsLabel = coverIncluded
    ? `${sheetsPerCopy} أوراق شامل الغلاف`
    : `${sheetsPerCopy} أوراق غير شامل الغلاف`;
  ...
  const totalProducedPages = useMemo(() => results.reduce((s, r) => s + r.pagesProduced, 0), [results]);
  ...
  const handleSave = async () => {
    ...
    try {
      const quoteData: Record<string, any> = {
        sourceType: 'newmagazine',
        rawInputs: { sheets, copies, totalPagesInput, coverIncluded },
      ...
      if (editingQuoteId) {
        ...
        const saved = await saveQuote(sessionToken, { title, customer_name: itemName, quote_number: itemNumber, source_type: 'newmagazine', quote_data: quoteData });
    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-4" dir="rtl">
      <div className="lg:col-span-2 space-y-4">
        ...
        <Card className="border-primary/20 shadow-sm">
          <CardContent className="p-4">
            <div className="flex items-center gap-3 mb-4">
              ...
              <h3 className="font-semibold text-foreground text-sm">بيانات المجلة</h3>
            </div>
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
              ...
              <div className="space-y-1">
                <Label className="text-xs">عدد صفحات كل نسخة</Label>
                <Input type="number" value={totalPagesInput} onChange={e => setTotalPagesInput(Number(e.target.value))} min={1} onFocus={e => e.target.select()} />
                {totalPagesInput !== actualPages && (
                  <p className="text-[10px] text-amber-600">تم الجبر إلى {actualPages} صفحة (يجب أن تقبل القسمة على 4)</p>
                )}
              </div>
              <div className="space-y-1">
                ...
                <div className="h-9 flex items-center px-3 rounded-md border bg-muted/30 text-sm">
                  <span className="font-semibold">{sheetsPerCopy}</span>
                </div>
                <div className="flex items-center gap-2 mt-1">
                  <Switch checked={coverIncluded} onCheckedChange={setCoverIncluded} className="scale-75" />
                  <Label className="text-[10px] text-muted-foreground">{coverIncluded ? 'شامل الغلاف' : 'غير شامل الغلاف'}</Label>
                </div>
                <p className="text-[10px] text-primary font-semibold">{sheetsLabel}</p>
              </div>
            </div>
          </CardContent>
        </Card>
        ...
        {rows.map((row, idx) => {
          ...
          const paperKey = row.paperType && row.grammage && row.purchaseSize ? `${row.paperType}|${row.grammage}|${row.purchaseSize}` : '';
          const enabledFinishing = row.finishing.filter(f => f.enabled);
          // Pages produced = sheets * 4
          const rowPageCount = res.pagesProduced * 4;
              ...
              <CardContent className="p-4 space-y-3">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-12 gap-3">
                  ...
                  <div className="space-y-1 sm:col-span-5">
                    ...
                    <ColorSelector sizes={sizes} selectedSizeIdx={row.sizeIdx} colorCount={row.colorCount} onSelect={handleColorSelect} />
                  </div>
                ...
                <div className="grid grid-cols-2 gap-2 text-[11px]">
                  ...
                  <div className="rounded-md bg-primary/5 border border-primary/20 px-2 py-1.5 text-center">
                    ...
                    <p className="font-bold text-primary">{row.cutsPerSheet} ورقة (= {row.cutsPerSheet * 4} صفحة)</p>
                  </div>
                </div>
                ...
                <div className="mt-2 pt-2 border-t border-border/50 grid grid-cols-2 sm:grid-cols-4 gap-2 text-xs">
                  <div className="bg-muted/40 rounded-lg p-2">
                    ...
                    <p className="font-bold text-foreground">{res.pagesProduced} أوراق ({res.pagesProduced * 4} صفحة)</p>
                  </div>
                  <div className="bg-muted/40 rounded-lg p-2">
        ...
        <Popover open={addRowPopoverOpen} onOpenChange={setAddRowPopoverOpen}>
          ...
          <PopoverContent className="w-48 p-2" align="center">
            <Button variant="ghost" size="sm" className="w-full justify-start text-xs gap-2" onClick={() => addRow(false)}>
              <Plus className="w-3.5 h-3.5" /> صف طباعة جديد
            </Button>
      ...
      <div className="space-y-4 lg:sticky lg:top-4 lg:self-start">
          ...
          <CardContent className="p-4 space-y-3">
            ...
            <div className="space-y-2 text-sm">
              ...
              <div className="flex justify-between">
                ...
                <span className="font-semibold">{actualPages} صفحة ({sheetsPerCopy} أوراق)</span>
              </div>
              <div className="flex justify-between">
                <span className="text-muted-foreground">إجمالي الأوراق المنتجة</span>
                <span className="font-semibold">{totalProducedPages}</span>
              </div>
              ...
              {results.map((r, i) => {
                ...
                return (
                  <div key={i} className="flex justify-between">
                    <span className="text-muted-foreground">{size?.sizeName || `صف ${i + 1}`}</span>
        ...
        {results.map((r, i) => {
          ...
          details.push({ label: 'أوراق من الشيت', value: `${r.pagesProduced} أوراق (${r.pagesProduced * 4} صفحة)` });
        ...
        <DialogContent dir="rtl" className="sm:max-w-md">
          ...
          <div className="space-y-3">
            <div className="space-y-1.5">
              ...
              <Input value={itemName} onChange={e => setItemName(e.target.value)} placeholder="مثال: مجلة تعريفية" className="h-9 text-sm" />
            </div>
            <div className="space-y-1.5">
...
export default NewMagazineCalculator;
