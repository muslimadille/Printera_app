// D001 — Sheet Nesting Preview
// Visualizes the sheet, margins, gripper, and the arrayed templates
// (with CUT/CREASE lines) using the active dynamic geometry and the
// computed nesting result. Preview only — does not affect calculations.

import { useMemo } from 'react';
import type { D001Params } from '@/lib/d001/types';
import { usableSheet } from '@/lib/d001/types';
import { buildD001Geometry, type Segment } from '@/lib/d001/geometry';
import type { D001NestingParams, D001NestingResult } from '@/lib/d001/nesting';
...
const D001SheetNestingPreview = ({ params, nesting, result }: Props) => {
  ...
  // Place gripper at the top of the sheet (printable area starts below it).
  ...
  // Item footprint depending on orientation (BBox, not pitch).
  const cellW = orientation === 'rotated' ? tH : tW;
  const cellH = orientation === 'rotated' ? tW : tH;
  // Pitch = real start-to-start distance (already accounts for interlock + gap).
  ...
  for (let r = 0; r < grid.rows; r++) {
    ...
    // Match the engine's brick clamp so the first item of a shifted row
    // never goes out of the usable strip.
  ...
  // Stroke widths scale with sheet size; keep readable but thin.
    ...
    <div className="space-y-2">
      <div className="border rounded-lg p-3 bg-white overflow-auto flex justify-center">
        ...
        >
          ...
          {items.map((it, i) => {
            // For rotated orientation: rotate template (tW × tH) by 90° so it
            // occupies (tH × tW). Translate by (x + tH, y) to land at cell origin.
...
export default D001SheetNestingPreview;
