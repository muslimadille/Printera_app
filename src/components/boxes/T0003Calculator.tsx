// T0003 — Dynamic Dieline tab (Mailer Box - Roll End Tuck Top)
...
import {
  ...
  type T0003Geometry,
  usableSheet
} from '@/lib/t0003/types';
import { buildT0003Geometry } from '@/lib/t0003/geometry';
import { buildT0003DimensionsSvg } from '@/lib/t0003/dimensionsOverlay';
import { computeT0003Nesting, type T0003NestingParams, type RotationMode } from '@/lib/t0003/nesting';
import T0003SheetNestingPreview from './T0003SheetNestingPreview';
import T0003PrintSummary from './T0003PrintSummary';
...
const DEFAULT_NESTING: T0003NestingParams = {
  horizontalGap: 3,
  verticalGap: 3,
  allowRotation: true,
  rotationMode: 'auto',
...
const T0003Calculator = ({ isAdmin = false, onSaveBox }: { isAdmin?: boolean; onSaveBox?: (box: any) => void }) => {
  const hiddenCls = isAdmin ? '' : 'hidden';

  const [params, setParams] = useState<T0003Params>(T0003_DEFAULTS);
  const [nesting, setNesting] = useState<T0003NestingParams>(DEFAULT_NESTING);
  ...
  const setN = <K extends keyof T0003NestingParams>(k: K, v: T0003NestingParams[K]) =>
    setNesting(prev => ({ ...prev, [k]: v }));

  const set = <K extends keyof T0003Params>(k: K, v: T0003Params[K]) => {
    setParams(prev => ({ ...prev, [k]: v }));
    setSegmentOverrides({ svg: null, segments: null });
  };

  const reset = () => {
    setParams({ ...T0003_DEFAULTS });
    setNesting({ ...DEFAULT_NESTING });
    setSegmentOverrides({ svg: null, segments: null });
  };

  const usable = useMemo(() => usableSheet(params), [params]);
  const nestingResult = useMemo(() => computeT0003Nesting(params, nesting), [params, nesting]);

  const baseGeo = useMemo(() => buildT0003Geometry(params), [params]);

  const geo = useMemo<T0003Geometry>(() => {
  ...
  const dimsSvg = useMemo(
    () => (showDimensions ? buildT0003DimensionsSvg(params, dimUnit, 1) : ''),
    ...
    <div dir="rtl" className="space-y-4">
      <Card>
        <CardHeader className="flex flex-row items-center justify-between">
          <CardTitle className="text-lg">T0003 — علبة بريدية (Mailer Box / Roll End Tuck Top)</CardTitle>
          <div className="flex items-center gap-3">
            <Label htmlFor="t0003-ref" className="text-sm font-normal cursor-pointer">
              وضع المرجعية Reference Mode {refOn && <span className="text-emerald-600">(مفعّل)</span>}
            </Label>
            <Switch id="t0003-ref" checked={refOn}
        ...
        {refOn && (
          <CardContent>
            <div className="rounded-md bg-amber-50 border border-amber-200 p-3 text-sm text-amber-900">
              وضع المرجعية مفعّل: الأبعاد الافتراضية مقفلة للمعايرة (W={T0003_REFERENCE.width}، H={T0003_REFERENCE.height}،
              D={T0003_REFERENCE.depth} مم).
            </div>
          </CardContent>
        )}
      </Card>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            <div className="sm:col-span-2 flex flex-col gap-2">
              ...
              <div className="flex items-center gap-2 h-10">
                <Switch id="t0003-smart" checked={!!nesting.smartAuto}
                  onCheckedChange={v => setN('smartAuto', v)} />
                <Label htmlFor="t0003-smart" className="text-sm font-normal">
          ...
          <div className="flex gap-2">
            ...
            <div className="flex items-center gap-2 pl-3 ml-1 border-l border-input">
              {previewMode === 'template' && (
                <>
                  <Switch id="t0003-show-dims" checked={showDimensions} onCheckedChange={setShowDimensions} />
                  <Label htmlFor="t0003-show-dims" className="text-sm font-normal cursor-pointer">إظهار القياسات</Label>
                </>
              )}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-[1fr_440px] gap-4 items-start">
            <div className="min-w-0">
              {previewMode === 'template' ? (
                ...
                <T0003SheetNestingPreview params={params} nesting={nesting} result={nestingResult} />
              ) : (
                <MailerBox3DPreview
                  svgMarkup={geo.svg}
                  svgWidth={geo.bbox.w}
              ...
              <section className="space-y-3">
                ...
                <div className="space-y-2">
                  <div className="flex items-center justify-between">
                    <Label htmlFor="t0003-allow-rot" className="text-xs">السماح بتدوير التصميم 90°</Label>
                    <Switch id="t0003-allow-rot" checked={nesting.allowRotation} onCheckedChange={v => setN('allowRotation', v)} />
                  </div>
                  {nesting.allowRotation && (
                    <div className="flex items-center justify-between gap-2">
                      <Label htmlFor="t0003-rot-mode" className="text-xs">وضع التدوير</Label>
                      <select id="t0003-rot-mode" className="h-8 rounded-md border border-input bg-background px-2 text-sm"
...
const ExportSingleButton = ({ params, geo }: { params: T0003Params; geo: T0003Geometry }) => {
  const onExportSvg = async () => {
    const { downloadT0003SingleTemplate } = await import('@/lib/t0003/exportSingle');
    const name = `T0003_${params.width}x${params.height}x${params.depth}.svg`;
    downloadT0003SingleTemplate(geo, name);
  };
  const onExportPdf = async () => {
    const { buildT0003SingleTemplateSvg, downloadT0003SingleTemplatePdf } = await import('@/lib/t0003/exportSingle');
    const svg = buildT0003SingleTemplateSvg(geo);
    const name = `T0003_${params.width}x${params.height}x${params.depth}.pdf`;
    await downloadT0003SingleTemplatePdf(svg, name);
  };
  return (
    <DropdownMenu>
...
const ExportSheetButton = ({
  ...
  result: ReturnType<typeof computeT0003Nesting>;
}) => {
  const disabled = result.fitStatus !== 'fits' || result.bestTotal <= 0;
  const onExportSvg = async () => {
    const { downloadT0003SheetLayout } = await import('@/lib/t0003/exportSheet');
    downloadT0003SheetLayout(params, nesting, result);
  };
  const onExportPdf = async () => {
    const { downloadT0003SheetLayoutPdf } = await import('@/lib/t0003/exportSheet');
    await downloadT0003SheetLayoutPdf(params, nesting, result);
  };
  return (
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
...
export default T0003Calculator;
