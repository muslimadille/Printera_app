...
interface T0005PrintSummaryProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  params: T0005Params;
  nesting: T0005NestingParams;
  nestingResult: T0005NestingResult;
  dimUnit: 'mm' | 'cm' | 'in';
      ...
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto print:max-w-none print:max-h-none print:overflow-visible print:shadow-none print:border-none">
        ...
        <div id="print-area" className="bg-background print:bg-white space-y-6" dir="rtl">
          ...
          <SectionBlock icon={<Ruler className="w-4 h-4" />} title="الألسنة واللصق">
            <table className="w-full text-sm border-collapse">
              <tbody>
                ...
                <PrintRow label="لسان القفل السفلي (Lid Tongue)" value={d(params.lidTongue)} />
                <PrintRow label="لسان الغبار السفلي (Dust Flap)" value={d(params.dustFlap)} />
              </tbody>
            </table>
          </SectionBlock>
          ...
          <SectionBlock icon={<Ruler className="w-4 h-4" />} title="الأبعاد المشتقة">
            <table className="w-full text-sm border-collapse">
              <tbody>
                <PrintRow label="Face Height = H" value={d(derived.faceHeight)} />
...
export default T0005PrintSummary;
