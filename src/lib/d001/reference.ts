// D001 — Reference Clone Geometry
// ----------------------------------------------------------------------------
// Every coordinate below is lifted from src/assets/D001/template.svg
...
import type { Pt, Segment, D001Geometry } from "./geometry";
import type { D001Params } from "./types";
import { D001_REFERENCE } from "./types";
...
// ============================================================================
export function buildD001Reference(params: D001Params): D001Geometry {
...
// ----------------------------------------------------------------------------
function finalize(params: D001Params, segs: Segment[]): D001Geometry {
  const all: Pt[] = [];
  for (const s of segs) {
  ...
  return {
    params,
    derived: {
      W: D001_REFERENCE.width,
      H: D001_REFERENCE.height,
      D: D001_REFERENCE.depth,
      glueFlap: D001_REFERENCE.glueFlap,
      faceHeight: D001_REFERENCE.height + 0.5,
      depth1: D001_REFERENCE.depth,
      depth2: D001_REFERENCE.depth - 0.5,
      coverVertical: D001_REFERENCE.depthTongue,
      lidCurveHeight: D001_REFERENCE.lidTongue,
      seg4Height: 5,
      width: bbox.w,
      height: bbox.h,
    },
...
function renderReferenceSvg(segs: Segment[], bbox: { w: number; h: number }): string {
  ...
  // CUT lives in its OWN group, painted LAST, so the small attached cuts
  // (48..55) cannot be visually overwritten by overlapping OUTER strokes.
  ...
  for (const s of segs.filter(s => s.kind === "OUTER")) {
    // Skip the 0-length spacer (Seg 15) from the rendered SVG to avoid degenerate paths.
  ...
  // CUT group painted LAST. Slightly thicker (0.35mm) so the 0.75 / 2.25 mm
  // attached cuts read clearly in any preview without being hidden by overlap.
  out.push(`  <g id="CUT" fill="none" stroke="${cutCol}" stroke-width="0.35" stroke-linecap="round" stroke-linejoin="round" opacity="1">`);
