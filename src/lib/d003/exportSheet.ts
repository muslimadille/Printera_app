...
//   - CUT: containing all OUTER (#ED1C24) and INNER_CUTS (#2028B0) lines

import type { D003Params } from './types';
import { usableSheet } from './types';
import { buildD003Geometry, type Segment } from './geometry';
import type { D003NestingParams, D003NestingResult } from './nesting';
import { computeD003Nesting } from './nesting';
...
export interface D003SheetExport {
  svg: string;
  filename: string;
  total: number;
}

export function buildD003SheetLayoutSvg(
  params: D003Params,
  nesting: D003NestingParams,
  resultIn?: D003NestingResult,
): D003SheetExport {
  const result = resultIn ?? computeD003Nesting(params, nesting);
  ...
  const filename = `D003-sheet-layout-W${sheetW}-H${sheetH}-QTY${result.bestTotal}.svg`;
  return { svg: out.join('\n'), filename, total: result.bestTotal };
}

export function downloadD003SheetLayout(
  params: D003Params,
  nesting: D003NestingParams,
  result?: D003NestingResult,
): void {
  const { svg, filename } = buildD003SheetLayoutSvg(params, nesting, result);
...
export async function buildD003SheetLayoutPdf(
  params: D003Params,
  nesting: D003NestingParams,
  result?: D003NestingResult,
) {
  const { svg, filename: svgFilename } = buildD003SheetLayoutSvg(params, nesting, result);
...
export async function downloadD003SheetLayoutPdf(
  params: D003Params,
  nesting: D003NestingParams,
  result?: D003NestingResult,
): Promise<void> {
  const { downloadPdf } = await import('@/lib/pdf/pdfService');
  const { pdf, filename } = await buildD003SheetLayoutPdf(params, nesting, result);
  downloadPdf(pdf, filename);
}

export async function previewD003SheetLayoutPdf(
  params: D003Params,
  nesting: D003NestingParams,
  result?: D003NestingResult,
): Promise<void> {
