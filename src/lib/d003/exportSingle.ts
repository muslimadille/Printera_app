...
export async function buildD003SingleTemplatePdf(svgMarkup: string) {
...
export async function downloadD003SingleTemplatePdf(
  ...
  const pdf = await buildD003SingleTemplatePdf(svgMarkup);
  downloadPdf(pdf, filename);
  return pdf;
}

export async function previewD003SingleTemplatePdf(
  svgMarkup: string,
  filename = "D003-single-template.pdf",
) {
