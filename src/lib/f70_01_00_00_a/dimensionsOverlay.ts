// F70_01_00_00_A — CAD Dimensions Overlay
// Generates professional CAD dimension lines and badged labels matching T0002 gold standard.

import type { F70_01_00_00_AGeometry } from './types';

export type DimUnit = 'mm' | 'cm' | 'in';

export function buildF70_01_00_00_ADimensionsSvg(
  geo: F70_01_00_00_AGeometry,
  unit: DimUnit = 'mm',
  scale = 1,
): string {
  const { derived } = geo;
  const { totalWidth, totalHeight, w, h, glueFlap, arcSagitta } = derived;
  ...
  const x1 = glueFlap;
  const x2 = glueFlap + w;
  const x3 = glueFlap + 2 * w;

  const yTopBase = arcSagitta;
  const yBotBase = arcSagitta + h;
  ...
  // Badged Label with white background for crystal clear readability
  ...
  // Horizontal dimension line with arrows and center badge
  ...
  // Vertical dimension line with arrows and center badge
  ...
  // 1. Panel 1 Width
  out += dimH(x1, x2, yTopBase + h * 0.28, fmt(w));

  // 2. Glue Flap Width
  out += dimH(x0, x1, yTopBase + h * 0.55, fmt(glueFlap));

  // 3. Body Height (centered in Panel 2)
  out += dimV((x2 + x3) / 2, yTopBase, yBotBase, fmt(h));
