// T0002 — Geometry Engine (Phase 2B)
// ----------------------------------------------------------------------------
// Two modes:
...
export interface Segment {
  ...
  geometry: "line" | "bezier" | "polyline" | "fillet" | string;
  start: Pt;
  end: Pt;
  bezier?: { c1: Pt; c2: Pt };
  points?: Pt[];
...
//                              PUBLIC API
// ============================================================================
export function buildT0002Geometry(p: T0002Params): T0002Geometry {
  if (p.referenceMode) return buildT0002Reference(p);
  return buildT0002Dynamic(p);
}

// ============================================================================
//                       DYNAMIC GEOMETRY MODE (Phase 2B)
// ============================================================================
export function buildT0002Dynamic(p: T0002Params): T0002Geometry {
  const W   = p.width;
  const H   = p.height;
  const D   = p.depth;
  const Gf  = p.glueFlap;
...
function finalize(
  params: T0002Params,
  segs: Segment[],
  base: Omit<T0002Derived, "width" | "height">,
): T0002Geometry {
  const all: Pt[] = [];
  for (const s of segs) {
