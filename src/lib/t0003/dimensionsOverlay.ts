// T0003 — Visual-only dimensions overlay.
...
export function buildT0003DimensionsSvg(p: T0003Params, unit: DimUnit = 'mm', scale = 1): string {
  const W = p.width;
  const H = p.height;
  const D = p.depth;
  const Gf = p.glueFlap;
  ...
  return `<g class="t0003-dimensions" pointer-events="none">${out}</g>`;
}
