...
// `T0003SheetNestingPreview`). Two layers only:
...
import { buildT0003Geometry, type Segment } from './geometry';
import type { T0003NestingParams, T0003NestingResult } from './nesting';
import { computeT0003Nesting } from './nesting';
...
export interface T0003SheetExport {
  svg: string;
  filename: string;
  total: number;
}

export function buildT0003SheetLayoutSvg(
  params: T0003Params,
  nesting: T0003NestingParams,
  resultIn?: T0003NestingResult,
): T0003SheetExport {
  const result = resultIn ?? computeT0003Nesting(params, nesting);
  ...
  const filename = `T0003-sheet-layout-W${sheetW}-H${sheetH}-QTY${result.bestTotal}.svg`;
  return { svg: out.join('\n'), filename, total: result.bestTotal };
}

export function downloadT0003SheetLayout(
  params: T0003Params,
  nesting: T0003NestingParams,
  result?: T0003NestingResult,
): void {
  const { svg, filename } = buildT0003SheetLayoutSvg(params, nesting, result);
...
export async function downloadT0003SheetLayoutPdf(
  params: T0003Params,
  nesting: T0003NestingParams,
  result?: T0003NestingResult,
): Promise<void> {
  const { svg, filename: svgFilename } = buildT0003SheetLayoutSvg(params, nesting, result);
  ...
  if (!svgEl) throw new Error('T0003 Sheet PDF export: failed to parse SVG.');
