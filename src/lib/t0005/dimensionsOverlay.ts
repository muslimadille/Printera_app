// T0005 — Visual-only dimensions overlay.
...
export function buildT0005DimensionsSvg(p: T0005Params, unit: DimUnit = 'mm', scale = 1): string {
  const W = p.width;
  const H = p.height;
  const D = p.depth;
  const Gf = p.glueFlap;
  ...
  return `<g class="t0005-dimensions" pointer-events="none">${out}</g>`;
}
