...
export default function AdminDashboard() {
  ...
  const initialTemplates = [
    { id: 'T00012', title: 'علبة بريدية بغطاء ملتف', category: 'retail', categoryLabel: 'تغليف تجزئة', downloads: 1420, active: true },
    { id: 'T0002', title: 'علبة مستقيمة الإغلاق', category: 'folding', categoryLabel: 'طي وصواني', downloads: 2890, active: true },
    { id: 'T0005', title: 'صندوق غطاء مفتوح بقفل', category: 'folding', categoryLabel: 'طي وصواني', downloads: 1980, active: true },
    { id: 'T0006', title: 'علبة قفل مزدوج الجدار', category: 'lidbase', categoryLabel: 'غطاء وقاعدة', downloads: 820, active: true },
    { id: 'D001-H', title: 'علبة كيك بمقبض حمل', category: 'retail', categoryLabel: 'تغليف تجزئة', downloads: 1100, active: true },
    { id: 'MED1', title: 'علبة دواء صغيرة', category: 'folding', categoryLabel: 'طي وصواني', downloads: 650, active: true },
  ];

  const openTemplateEdit = (t: typeof initialTemplates[0]) => {
    setSelectedTemplateId(t.id);
    setTemplateName(t.title);
    setTemplateCat(t.categoryLabel);
    setTemplateW('200');
    setTemplateH('120');
    setTemplateD('80');
  };
  ...
  return (
    <div dir="rtl" style={{ minHeight: '100vh', background: '#faf6f0', display: 'flex', fontFamily: 'Cairo, sans-serif' }}>
      
      {/* ===== Sidebar ===== */}
      <aside style={{ width: '240px', background: '#2b2013', color: '#f3ece0', display: 'flex', flexDirection: 'column', flexShrink: 0, position: 'sticky', top: 0, height: '100vh' }}>
        <div style={{ padding: 'var(--space-6) var(--space-5)', borderBottom: '1px solid #463a29', display: 'flex', alignItems: 'center', justifyContext: 'space-between', justifyContent: 'space-between' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            <span style={{ width: '30px', height: '30px', borderRadius: 'var(--radius-sm)', background: '#a9622f', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontSize: '15px', fontWeight: 700 }}>ق</span>
            <span style={{ fontWeight: 700, fontSize: '16px', color: '#fff' }}>قوالب<span style={{ color: '#c98a52' }}>لاين</span></span>
          </div>
          <span style={{ background: '#463a29', color: '#d8cbb5', fontSize: '10px', fontWeight: 700, padding: '3px 8px', borderRadius: '4px' }}>أدمن</span>
        </div>
            ...
            return (
              <button
                ...
                style={{
                  ...
                  background: active ? '#463a29' : 'transparent',
        ...
        <div style={{ padding: 'var(--space-4)', borderTop: '1px solid #463a29', display: 'flex', alignItems: 'center', gap: '10px' }}>
          <div style={{ width: '34px', height: '34px', borderRadius: '50%', background: '#a9622f', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 700, fontSize: '14px', color: '#fff' }}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minWidth: 0 }}>
        ...
        {/* Top Header */}
        <header style={{ background: '#faf6f0', borderBottom: '1px solid #e8ded0', padding: '14px var(--space-8)', display: 'flex', alignItems: 'center', justifyContext: 'space-between', justifyContent: 'space-between', position: 'sticky', top: 0, zIndex: 10 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
            <div style={{ position: 'relative', width: '220px' }}>
              <i className="ph ph-magnifying-glass" style={{ position: 'absolute', right: '12px', top: '50%', transform: 'translateY(-50%)', fontSize: '14px', color: '#9c8f7c' }}></i>
              <input 
                type="text" 
                placeholder="بحث عام..."
                value={search}
                onChange={e => setSearch(e.target.value)}
                style={{ width: '100%', boxSizing: 'border-box', height: '36px', borderRadius: '999px', border: '1px solid #e6dccb', background: '#fff', padding: '0 34px 0 12px', fontSize: '13px', fontFamily: 'Cairo,sans-serif' }}
              />
            </div>
            <Link to="/" style={{ display: 'flex', alignItems: 'center', gap: '6px', border: '1px solid #e6dccb', background: '#fff', color: '#2b2013', fontWeight: 700, fontSize: '13px', padding: '8px 16px', borderRadius: '999px', textDecoration: 'none' }}>
        ...
        <main style={{ flex: 1, padding: 'var(--space-8)', maxWidth: '1180px', width: '100%', boxSizing: 'border-box' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-8)' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: 'var(--space-5)' }}>
                {[
                  ...
                  <div key={idx} className="hover-lift" style={{ background: '#fff', border: '1px solid #e6dccb', borderRadius: 'var(--radius-lg)', padding: 'var(--space-5)', display: 'flex', flexDirection: 'column', gap: '10px' }}>
                    <div style={{ display: 'flex', alignItems: 'center', justifyContext: 'space-between', justifyContent: 'space-between' }}>
                      <div style={{ width: '36px', height: '36px', borderRadius: '8px', background: s.iconBg, display: 'flex', alignItems: 'center', justifyContext: 'center', justifyContent: 'center' }}>
                        <i className={s.icon} style={{ fontSize: '18px', color: '#2b2013' }}></i>
                      </div>
                    ...
                    <div>
                      ...
                      <div style={{ fontSize: '13px', color: '#9c8f7c', marginTop: '2px' }}>{s.title}</div>
                    </div>
                  </div>
                ))}
              </div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1.6fr 1fr', gap: 'var(--space-6)', alignItems: 'start' }}>
                ...
                <div style={{ background: '#fff', border: '1px solid #e6dccb', borderRadius: 'var(--radius-lg)', overflow: 'hidden' }}>
                  <div style={{ padding: 'var(--space-4)', borderBottom: '1px solid #eee2cf', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                    <h3 style={{ margin: 0, fontSize: '16px', fontWeight: 700 }}>أحدث الطلبات</h3>
                    <button onClick={() => setActiveSection('orders')} style={{ border: 'none', background: 'transparent', color: '#a9622f', fontWeight: 700, fontSize: '13px', cursor: 'pointer' }}>عرض الكل</button>
                  </div>
                ...
                {/* New Users */}
                <div style={{ background: '#fff', border: '1px solid #e6dccb', borderRadius: 'var(--radius-lg)', overflow: 'hidden' }}>
                  <div style={{ padding: 'var(--space-4)', borderBottom: '1px solid #eee2cf' }}>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column' }}>
                    {recentUsers.map((u, idx) => (
                      <div key={idx} style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '12px 16px', borderBottom: idx < recentUsers.length - 1 ? '1px solid #f2e9d9' : 'none' }}>
                        <div style={{ width: '32px', height: '32px', borderRadius: '50%', background: '#f4ede1', color: '#a9622f', display: 'flex', alignItems: 'center', justifyContext: 'center', justifyContent: 'center', fontWeight: 700, fontSize: '13px' }}>
                          {u.name.trim().charAt(0)}
                        </div>
                        <div style={{ flex: 1 }}>
                          <div style={{ fontSize: '13.5px', fontWeight: 700 }}>{u.name}</div>
                          <div style={{ fontSize: '11px', color: '#9c8f7c', marginTop: '1px' }}>{u.email}</div>
                        </div>
                        <span style={{ fontSize: '11.5px', color: '#9c8f7c' }}>{u.date}</span>
                      </div>
                    ))}
                  </div>
                </div>
          ...
          {activeSection === 'templates' && (
            <div>
              {!selectedTemplateId ? (
                <div style={{ background: '#fff', border: '1px solid #e6dccb', borderRadius: 'var(--radius-lg)', overflow: 'hidden' }}>
                  <div style={{ padding: 'var(--space-4)', borderBottom: '1px solid #eee2cf', display: 'flex', alignItems: 'center', justifyContext: 'space-between', justifyContent: 'space-between' }}>
                    <h3 style={{ margin: 0, fontSize: '16px', fontWeight: 700 }}>إدارة القوالب</h3>
                    <button onClick={() => toast.info('يرجى استخدام شروحات create-packaging-template لإضافة قالب جديد.')} style={{ background: '#2b2013', color: '#fff', border: 'none', padding: '8px 16px', borderRadius: '999px', fontSize: '13px', fontWeight: 700, cursor: 'pointer' }} className="btn-anim">
                      ...
                      {initialTemplates.map((t) => (
                        <tr key={t.id} className="row-hover">
                          <td style={{ ...styleTd, fontWeight: 700 }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                              <i className="ph ph-cube" style={{ color: '#a9622f' }}></i>
                              <span>{t.title} ({t.id})</span>
                            </div>
                          </td>
                          <td style={styleTd}>{t.categoryLabel}</td>
                          <td style={styleTd}>{t.downloads}</td>
                          <td style={styleTd}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1.3fr', gap: 'var(--space-6)', alignItems: 'start' }}>
                  {/* Left visual */}
                  <div style={{ background: '#fff', border: '1px solid #e6dccb', borderRadius: 'var(--radius-lg)', padding: 'var(--space-6)', textAlign: 'center' }}>
                    <h3 style={{ margin: '0 0 10px', fontSize: '15px', fontWeight: 700, color: '#9c8f7c' }}>معاينة قالب {selectedTemplateId}</h3>
                    <div style={{ background: '#faf6f0', borderRadius: '8px', padding: '20px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                      <img src={`/templates/preview/A10_20_02_02.svg`} alt="معاينة" style={{ width: '80%', height: 'auto' }} />
                    </div>
                  </div>

                  {/* Right edit form */}
                  <div style={{ background: '#fff', border: '1px solid #e6dccb', borderRadius: 'var(--radius-lg)', padding: 'var(--space-6)' }}>
                    <div style={{ display: 'flex', alignItems: 'center', justifyContext: 'space-between', justifyContent: 'space-between', marginBottom: 'var(--space-5)', borderBottom: '1px solid #eee2cf', paddingBottom: '10px' }}>
                      <h3 style={{ margin: 0, fontSize: '18px', fontWeight: 700 }}>تعديل قالب {selectedTemplateId}</h3>
                      <button onClick={() => setSelectedTemplateId(null)} style={{ border: 'none', background: 'transparent', color: '#8a7d6d', cursor: 'pointer', fontSize: '13.5px', fontWeight: 600 }}>إغلاق</button>
                    </div>

                    <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
                      <div>
                        <label style={{ display: 'block', fontSize: '13px', fontWeight: 600, marginBottom: '6px' }}>اسم القالب</label>
                        <input type="text" value={templateName} onChange={e => setTemplateName(e.target.value)} style={{ width: '100%', boxSizing: 'border-box', height: '40px', borderRadius: '6px', border: '1px solid #e6dccb', padding: '0 12px', fontSize: '13.5px', fontFamily: 'Cairo,sans-serif' }} />
                      </div>
                      <div>
                        <label style={{ display: 'block', fontSize: '13px', fontWeight: 600, marginBottom: '6px' }}>التصنيف</label>
                        <input type="text" value={templateCat} onChange={e => setTemplateCat(e.target.value)} style={{ width: '100%', boxSizing: 'border-box', height: '40px', borderRadius: '6px', border: '1px solid #e6dccb', padding: '0 12px', fontSize: '13.5px', fontFamily: 'Cairo,sans-serif' }} />
                      </div>
                      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '10px' }}>
                        <div>
                          <label style={{ display: 'block', fontSize: '11.5px', color: '#5a4c3c', marginBottom: '4px' }}>العرض الافتراضي</label>
                          <input type="number" value={templateW} onChange={e => setTemplateW(e.target.value)} style={{ width: '100%', boxSizing: 'border-box', height: '40px', borderRadius: '6px', border: '1px solid #e6dccb', padding: '0 12px', fontSize: '13.5px' }} />
                        </div>
                        <div>
                          <label style={{ display: 'block', fontSize: '11.5px', color: '#5a4c3c', marginBottom: '4px' }}>الارتفاع الافتراضي</label>
                          <input type="number" value={templateH} onChange={e => setTemplateH(e.target.value)} style={{ width: '100%', boxSizing: 'border-box', height: '40px', borderRadius: '6px', border: '1px solid #e6dccb', padding: '0 12px', fontSize: '13.5px' }} />
                        </div>
                        <div>
                          <label style={{ display: 'block', fontSize: '11.5px', color: '#5a4c3c', marginBottom: '4px' }}>العمق الافتراضي</label>
                          <input type="number" value={templateD} onChange={e => setTemplateD(e.target.value)} style={{ width: '100%', boxSizing: 'border-box', height: '40px', borderRadius: '6px', border: '1px solid #e6dccb', padding: '0 12px', fontSize: '13.5px' }} />
                        </div>
                      </div>

                      <div style={{ display: 'flex', gap: '10px', marginTop: '10px' }}>
                        <button onClick={handleSaveTemplate} style={{ flex: 1, height: '44px', border: 'none', borderRadius: '999px', background: '#2b2013', color: '#fff', fontWeight: 700, fontSize: '14px', cursor: 'pointer' }} className="btn-anim">حفظ التغييرات</button>
          ...
          {activeSection === 'users' && (
            <div>
              ...
              <div style={{ display: 'flex', gap: '6px', borderBottom: '1px solid #e8ded0', marginBottom: 'var(--space-6)' }}>
                <button type="button" onClick={() => setUsersTab('list')} style={{ padding: '10px 16px', border: 'none', background: 'transparent', cursor: 'pointer', fontFamily: 'Cairo', fontSize: '13.5px', fontWeight: usersTab === 'list' ? 700 : 500, color: usersTab === 'list' ? '#2b2013' : '#9c8f7c', borderBottom: `2px solid ${usersTab === 'list' ? '#a9622f' : 'transparent'}`, marginBottom: '-1px' }}>المستخدمون</button>
                <button type="button" onClick={() => setUsersTab('roles')} style={{ padding: '10px 16px', border: 'none', background: 'transparent', cursor: 'pointer', fontFamily: 'Cairo', fontSize: '13.5px', fontWeight: usersTab === 'roles' ? 700 : 500, color: usersTab === 'roles' ? '#2b2013' : '#9c8f7c', borderBottom: `2px solid ${usersTab === 'roles' ? '#a9622f' : 'transparent'}`, marginBottom: '-1px' }}>الأدوار والصلاحيات</button>
              </div>

              {usersTab === 'list' ? (
                <div style={{ background: '#fff', border: '1px solid #e6dccb', borderRadius: 'var(--radius-lg)', padding: 'var(--space-4)' }}>
                  <UserManagement 
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-6)' }}>
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: 'var(--space-5)' }}>
                    {[
                      ...
                      <div key={idx} style={{ background: '#fff', border: '1px solid #e6dccb', borderRadius: 'var(--radius-lg)', padding: 'var(--space-6)' }}>
                        <div style={{ display: 'flex', alignItems: 'center', justifyContext: 'space-between', justifyContent: 'space-between', marginBottom: '14px' }}>
                          ...
                          <span style={{ fontSize: '12px', color: '#9c8f7c' }}>{r.count} مستخدم</span>
                        </div>
                        <div style={{ fontSize: '13px', color: '#8a7d6d', marginBottom: 'var(--space-4)' }}>{r.label}</div>
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                          {r.permissions.map((p, pIdx) => (
                            <label key={pIdx} style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '13px', color: '#2b2013' }}>
                              <input type="checkbox" checked={r.active[pIdx]} readOnly disabled style={{ accentColor: '#a9622f' }} />
                              {p}
                            </label>
                          ))}
                        </div>
                  ...
                  <button onClick={() => toast.info('إضافة دور جديد تتطلب ترقية خطة الاستضافة الخاصة بالنظام.')} style={{ border: '2px dashed #d8cbb5', borderRadius: '12px', padding: '16px', background: 'transparent', color: '#a9622f', fontWeight: 700, fontSize: '14px', cursor: 'pointer', textAlign: 'center' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-6)' }}>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: 'var(--space-4)' }}>
                {[
                  ...
                  <div key={idx} style={{ background: '#fff', border: '1px solid #e6dccb', borderRadius: 'var(--radius-lg)', padding: 'var(--space-5)' }}>
                    <div style={{ fontSize: '22px', fontWeight: 700 }}>{b.value}</div>
                    <div style={{ fontSize: '13px', color: '#9c8f7c', marginTop: '2px' }}>{b.title}</div>
                  </div>
                ))}
              </div>
          ...
          {activeSection === 'settings' && (
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: 'var(--space-6)' }}>
              ...
              {/* general */}
              <div style={{ background: '#fff', border: '1px solid #e6dccb', borderRadius: 'var(--radius-lg)', padding: 'var(--space-6)', display: 'flex', flexDirection: 'column', gap: '14px' }}>
                <h3 style={{ margin: 0, fontSize: '16px', fontWeight: 700, borderBottom: '1px solid #eee2cf', paddingBottom: '8px' }}>عام</h3>
                <div>
                  <label style={{ display: 'block', fontSize: '13px', fontWeight: 600, marginBottom: '6px' }}>اسم المنصة</label>
                  <input type="text" defaultValue="قوالبلاين" style={{ width: '100%', boxSizing: 'border-box', height: '40px', borderRadius: '6px', border: '1px solid #e6dccb', padding: '0 12px', fontSize: '13.5px' }} />
                </div>
                <div>
                  <label style={{ display: 'block', fontSize: '13px', fontWeight: 600, marginBottom: '6px' }}>بريد الدعم</label>
                  <input type="email" defaultValue="support@qawalibline.sa" style={{ width: '100%', boxSizing: 'border-box', height: '40px', borderRadius: '6px', border: '1px solid #e6dccb', padding: '0 12px', fontSize: '13.5px' }} />
                </div>
                <div>
                  <label style={{ display: 'block', fontSize: '13px', fontWeight: 600, marginBottom: '6px' }}>العملة الافتراضية</label>
                  <input type="text" defaultValue="ر.س" style={{ width: '100%', boxSizing: 'border-box', height: '40px', borderRadius: '6px', border: '1px solid #e6dccb', padding: '0 12px', fontSize: '13.5px' }} />
                </div>
              </div>

              {/* preview settings */}
              <div style={{ background: '#fff', border: '1px solid #e6dccb', borderRadius: 'var(--radius-lg)', padding: 'var(--space-6)', display: 'flex', flexDirection: 'column', gap: '14px' }}>
                <h3 style={{ margin: 0, fontSize: '16px', fontWeight: 700, borderBottom: '1px solid #eee2cf', paddingBottom: '8px' }}>إعدادات المعاينة</h3>
                <label style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '13.5px', color: '#2b2013', cursor: 'pointer' }}>
              ...
              {/* categories */}
              <div style={{ background: '#fff', border: '1px solid #e6dccb', borderRadius: 'var(--radius-lg)', padding: 'var(--space-6)', display: 'flex', flexDirection: 'column', gap: '14px' }}>
                <h3 style={{ margin: 0, fontSize: '16px', fontWeight: 700, borderBottom: '1px solid #eee2cf', paddingBottom: '8px' }}>تصنيفات القوالب</h3>
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px' }}>
                  {['طي وصواني', 'علب أسطوانية', 'تغليف تجزئة', 'علب سحب', 'أشكال غير مستطيلة'].map((cat, idx) => (
                    <span key={idx} style={{ background: '#f4ede1', color: '#5a4c3c', fontSize: '12px', padding: '5px 12px', borderRadius: '6px', display: 'flex', alignItems: 'center', gap: '6px' }}>
                      {cat}
                      <i className="ph ph-x" style={{ cursor: 'pointer' }} onClick={() => toast.error('لا يمكن حذف التصنيف الأساسي')}></i>
                    </span>
                  ))}
                </div>
                <button onClick={() => toast.info('إضافة تصنيف يتطلب ضبط ملفات القوالب البرمجية أولاً.')} style={{ border: '1px dashed #d8cbb5', borderRadius: '8px', padding: '10px', background: 'transparent', color: '#a9622f', fontWeight: 700, fontSize: '13px', cursor: 'pointer', textAlign: 'center' }}>
              ...
              {/* notification */}
              <div style={{ background: '#fff', border: '1px solid #e6dccb', borderRadius: 'var(--radius-lg)', padding: 'var(--space-6)', display: 'flex', flexDirection: 'column', gap: '14px' }}>
                <h3 style={{ margin: 0, fontSize: '16px', fontWeight: 700, borderBottom: '1px solid #eee2cf', paddingBottom: '8px' }}>الإشعارات</h3>
                {[
                  ...
                  <label key={idx} style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '13.5px', color: '#2b2013', cursor: 'pointer' }}>
                    <input type="checkbox" defaultChecked={n.defaultChecked} style={{ accentColor: '#a9622f' }} />
                    {n.label}
                  </label>
                ))}
              </div>
              ...
              <div style={{ background: '#fff', border: '1px solid #c0392b', borderRadius: 'var(--radius-lg)', padding: 'var(--space-6)', display: 'flex', flexDirection: 'column', gap: '14px' }}>
                ...
                <p style={{ fontSize: '13px', color: '#8a7d6d', margin: 0, lineHeight: 1.5 }}>
              ...
              <div style={{ gridColumn: '1 / -1', display: 'flex', justifyContent: 'flex-end', marginTop: '10px' }}>
                <button onClick={() => toast.success('تم حفظ إعدادات النظام بنجاح')} style={{ height: '46px', border: 'none', borderRadius: '999px', background: '#2b2013', color: '#fff', fontWeight: 700, fontSize: '14px', padding: '0 32px', cursor: 'pointer' }} className="btn-anim">حفظ الإعدادات</button>
              </div>
