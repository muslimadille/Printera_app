...
for (const line of lines) {
  ...
  if (currentSheet === 'Dynamic_Lines' && cols.length >= 2 && cols[0] !== 'Line_ID' && cols[0].trim() !== '') {
    dynamicLines.push({ id: cols[0], layer: cols[1], p1: cols[3], p2: cols[4] });
  }
}
...
export function buildT0007Geometry(p: T0007Params): T0007Geometry {
  ...
  const pts: Record<string, Pt> = {
    ...
    P011: { x: X_Left_Main + 12.5, y: 0.35 },
    P012: { x: X_Right_Main - 12.5, y: 0.35 },
    ...
    P021: { x: X_Right_Main - 0.5, y: 12.35 },
